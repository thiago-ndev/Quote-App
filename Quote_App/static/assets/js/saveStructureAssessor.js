...
const tableRows = document.querySelectorAll(".inv--product-table-section table tbody tr");
  ...
  tableRows.forEach(function(row) {
    ...
    for (let i = 0; i < 6; i++) {
      if (i==2){
        console.log(rowCells[i].textContent.trim().split('-')[0].replace(' ', ''))
        rowData.push(rowCells[i].textContent.trim().split('-')[0].replace(' ', ''));
      } else if (i==3){
        rowData.push(rowCells[i].querySelector('.due-date-dropdown').value);

      } else if (i==4){
        rowData.push(rowCells[2].textContent.trim().split('-')[1].replace(' ', ''));
